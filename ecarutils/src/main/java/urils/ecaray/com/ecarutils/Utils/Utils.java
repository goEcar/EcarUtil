...
public class Utils {
    ...
    //复制到剪贴板
    private static void toPatch(Context context, String imeiNum) {
        ClipboardManager cmb = (ClipboardManager) context.getSystemService(Service.CLIPBOARD_SERVICE);
        cmb.setText(imeiNum);
    }
    ...
    public static String getImeiNum(Context context) {
        ...
        try {
            ...
            imeiNum = null;
//            Toast.makeText(this, "IMEI号获取失败!", Toast.LENGTH_LONG).show();

        }
        if (imeiNum != null) {
            toPatch(context, imeiNum);
//            Toast.makeText(this, "已经将IMEI号复制到了剪贴板!", Toast.LENGTH_LONG).show();
        }
        return imeiNum;
    }
