...
public class ViewHolderBee {
	...
	public static ViewHolderBee get(Context context, View convertView,
		...
		if (convertView == null) {
			...
			ViewHolderBee holder = (ViewHolderBee) convertView.getTag();
	...
	public ViewHolderBee setTextQuery(int viewId, String text, String query,
									  int color) {
		TextView tv = getView(viewId);
		StringsUtil.setSearchKey(tv, text, query, color);
		return this;
	}
	...
	public ViewHolderBee setTextColor(int viewId,int color) {
		TextView tv = getView(viewId);
		tv.setTextColor(color);
		return this;
	}
	...
	public ViewHolderBee setViewsVisible(int[] ids,boolean isVisible){
		int visibleId = isVisible ? View.VISIBLE : View.INVISIBLE;
		int length = ids.length;
		for(int i=0;i<length;i++){
			getView(ids[i]).setVisibility(visibleId);
		}
		return this;
	}
